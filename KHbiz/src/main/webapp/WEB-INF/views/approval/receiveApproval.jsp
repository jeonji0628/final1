    	...
    	<div id="page-inner"> 
    		<div id="center">
	    		<ul class="nav nav-tabs">
				   ...
				   <li role="presentation" class="active"><a href="#">받은 결재함</a></li>   
				</ul>
			</div>
			<div id="main">
			
				<c:if test="${result ne null}">
					<div id="result">받은 결재함이 없습니다</div>
				</c:if>
		                		...
		                		<tr>
			                		...
			                		<td>
			                			<c:if test="${approval.status eq '대기중'}">
			                				...
			                				<button data-toggle="modal" value="${i.index}" data-target="#delete" class="btn btn-danger approval-delete">반려</button> 
			                			</c:if>
					  ...
					  <ul class="pagination pagination-lg">
					  	<li>
					    	<c:if test="${paging.curblock > 1}">
					    		<a href="receiveApproval?curpage=${paging.startnum -1}&id=${member.id}" aria-label="Previous">
					    ...
					    <c:forEach begin="${paging.startnum}" end="${paging.lastnum}" step="1" var="i">
					    	<li><a href="receiveApproval?curpage=${i}&id=${member.id}">${i}</a></li>
					    </c:forEach>
					    
					    <c:if test="${paging.curblock < paging.totalblock }">
						    <li>
						      <a href="receiveApproval?curpage=${paging.lastnum + 1}&id=${member.id}" aria-label="Next">
