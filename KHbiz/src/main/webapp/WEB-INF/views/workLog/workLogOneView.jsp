			 ...
			 <div class="table-up">
				<ul class="nav nav-tabs">
						<li role="presentation" class="active">
							<c:if test="${workLogOne.state eq 1}">
								<h3>일일업무일지보기</h3>
							</c:if>
							<c:if test="${workLogOne.state eq 2}">
								<h3>주간업무일지보기</h3>
							</c:if>
							<c:if test="${workLogOne.state eq 3}">
								<h3>월간업무일지보기</h3>
							</c:if>
						</li>
					</ul>
			...
			<div class="table-responsive2">
	               <table  class="table">
	        			...
	        			<td id="board_input" style="padding-top: 13px;">
	                   		<div class="col-lg-6">
							    <div class="input-group">
							      ${workLogOne.division} 
							      <c:if test="${workLogOne.grade == 1}">
									사원
									</c:if>
									<c:if test="${workLogOne.grade == 2}">
									주임
									</c:if>
									<c:if test="${workLogOne.grade == 3}">
									대리
									</c:if>
									<c:if test="${workLogOne.grade == 4}">
									과장
									</c:if>
									<c:if test="${workLogOne.grade == 5}">
									차장
									</c:if>
									<c:if test="${workLogOne.grade == 6}">
									부장
									</c:if>
									<c:if test="${workLogOne.grade == 7}">
									실장
									</c:if>
									<c:if test="${workLogOne.grade == 8}">
									이사
									</c:if>
									<c:if test="${workLogOne.grade == 9}">
									대표이사
									</c:if>									
							      ${member.id}    
							    </div>
	        			...
	        			<tr>
	                   	...
	                   	<!-- title -->
	                   	<td style="padding-top: 13px;"><input type="text" value="${workLogOne.reg_date}" name="reg_date" readonly="readonly" class="form-control"></td>
	                   </tr>
	                   <tr>
	                   <!-- contents -->
